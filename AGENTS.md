...
| `docs/operations/monitoring.md` | How failures get noticed: the triage table, probe policy and inventory, CronJob deadlines, the backup verification gates, the healthchecks.io checks that remain and how to list them live, the uptime-kuma push monitors and how to list them live, the disclosure rules for both, and what none of it catches |
...
- **Scheduled work gets a dead-man's-switch, not a probe.**
  ...
  **New scheduled work drives an uptime-kuma PUSH monitor, not a healthchecks.io check** (migrated 2026-08-26).
  The live inventory is one call with the read-only API key, and the command is in `docs/operations/monitoring.md`; each check's reason for keeping its slot is the table in that file.
  Never write the count into this file, because it has drifted twice.
