...
| 2026-08-20 | FreshRSS PubSubHubbub per-feed callback secrets (`k=` query parameter; FreshRSS-generated, not in 1Password) | Surfaced in a read-only traffic-analysis agent's tool output while reading `/var/www/FreshRSS/data/users/_/log_pshb.txt` and the Apache access log inside the `freshrss` pod. Never written to a file or committed. | Traffic-analysis agent (log investigation) | pending operator decision — these authenticate inbound WebSub push callbacks per feed. They rotate naturally by re-subscribing the affected feeds. Note the callbacks are currently 301-redirected and largely failing anyway (registered as http:// not https://) |
| 2026-08-20 | Luma feed API key (`?key=` parameter of the `visionary-swan.pikapod.net/luma/london-mlops` feed URL; not in 1Password) | Same event — appeared in the agent's tool output while reading `/var/www/FreshRSS/data/users/ruined0346/log.txt`. Never written to a file or committed. | Traffic-analysis agent (log investigation) | pending operator action — regenerate at source (Luma), then update the subscription URL in FreshRSS |
