...
| `withings-ingest` | `op://Homelab/health-healthchecks/withings-kuma-push-token` | 21600s, 1 retry at 7200s | `withings-ingest` CronJob in `health`, Python: `up` on rc 0, `down` otherwise. `msg` carries `verdict=` from `ok\|failed`, `groups=` and `points=`, plus `failure=` from a five-member enum, plus `exception=` after it on an unhandled error |
