...
- **Memory writes.** The agent's Hindsight writes have been failing with HTTP 401 since before this machinery existed, and nothing here detects it: the write is on a background path a chat response does not wait for. The diagnostic is in [the update runbook](hermes-vm-updates.md#first-session).
